...
public abstract class AbstractApplicationContext extends DefaultResourceLoader
	...
	ApplicationEventMulticaster getApplicationEventMulticaster() throws IllegalStateException {
		...
		return this.applicationEventMulticaster;//已经在前面初始化了，可以获取到
	}
	...
	public void refresh() throws BeansException, IllegalStateException {
		synchronized (this.startupShutdownMonitor) {
			...
			try {
				...
				//各种beanFactoryPostProcesser的执行过程
				...
				invokeBeanFactoryPostProcessors(beanFactory);//包扫描
				...
				//我们后续注册的所有监听器，在发布时间执行时，都是调用这个对象的getListenter方法来找到对应的监听器
				initApplicationEventMulticaster();

				// Initialize other special beans in specific context subclasses.
				// 执行其他的初始化操作，例如和SpringMVC整合时，需要初始化一些其他的bean，但是对于纯spring工程来说，onFresh方法是一个空方法
				//在springboot中，在此方法中进行tomcat的创建，即内嵌tomcat的原理，
				onRefresh();
				...
				// 为什么说是剩余呢？如果开发人员自定义了BeanPosrProcessor，而BeanPostProcessor在前面已经实例化了，在这里实例化的bean需要完成BeanPostProcessor中的操作，所以在这里不会再实例化，因此这里使用剩余一词
				finishBeanFactoryInitialization(beanFactory);

				// 结束refresh，主要干了一件事，就是发布一个事件ContextRefreshEvent，通知大家spring容器refresh结束了。
				//在web环境下  ContextRefreshEvent最终会进行disPatcherServlet的初始化
				finishRefresh();
			}
	 ...
	 * @see org.springframework.context.event.SimpleApplicationEventMulticaster
	 */
	//初始化applicationEventMulticaster的组件
	protected void initApplicationEventMulticaster() {
		ConfigurableListableBeanFactory beanFactory = getBeanFactory();
		//容器中是否存在applicationEventMulticaster
	...
	protected void finishRefresh() {
		...
		//发布一个完成事件
		//在web环境下  ContextRefreshEvent最终会进行disPatcherServlet的初始化,即init()
