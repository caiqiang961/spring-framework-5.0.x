...
class ConfigurationClassBeanDefinitionReader {
	...
	private void loadBeanDefinitionsForConfigurationClass(
		...
		for (BeanMethod beanMethod : configClass.getBeanMethods()) {
			//将其中的@Bean的方法注册到DefineMap中，在此时会执行如@ConditionalOnBean等注解，他们是在注册bean阶段进行的
			//@Conditional在俩个阶段都会解析，这也就是为什么@Conditional会解析好多次了
			loadBeanDefinitionsForBeanMethod(beanMethod);
		}

		//处理在解析@ImportResources注解时
		loadBeanDefinitionsFromImportedResources(configClass.getImportedResources());

		//处理在解析@Import注解时，其实现类为ImportBeanDefinitionRegistrar子类的registerBeanDefinitions()
